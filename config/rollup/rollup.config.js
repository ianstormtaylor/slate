...
function configure(pkg, env, target) {
  ...
  const dir = pkg.name.replace('-latest', '')
  console.log({ dir })
  ...
  if (isUmd) {
    return {
      ...
      output: {
        format: 'umd',
        file: `packages/${dir}/${isProd ? pkg.umdMin : pkg.umd}`,
        exports: 'named',
