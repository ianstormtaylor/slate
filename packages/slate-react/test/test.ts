...
describe('slate-react', () => {
  fixtures(__dirname, 'selection', ({ module }) => {
    // Arrange
    let { selection, slateRangeSelection, nextNodeEntry, output } = module

    // Act
    const result = testToSlateRange(selection, slateRangeSelection, nextNodeEntry)
...
const testToSlateRange = (inputSelection: DOMSelection, inputSlateRangeSelection: SlateRange, nextNodeEntry: number[] | undefined) => {
  ...
  const mockNextNode = jest.fn()
    .mockReturnValue(nextNodeEntry)
