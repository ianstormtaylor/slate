  ...
  Editor.withoutNormalizing(editor, () => {
    ...
    if (Range.isRange(at)) {
      ...
      if (Range.isCollapsed(at)) {
        ...
        Transforms.deleteContent(editor, { at })
        at = pointRef.unref()!
      }
    }
...
export function deleteRange(editor: Editor, range: Range): Point | null {
  if (Range.isCollapsed(range)) {
    return range.anchor
  } else {
    const [, end] = Range.edges(range)
    const pointRef = Editor.pointRef(editor, end)
    Transforms.deleteContent(editor, { at: range })
    return pointRef.unref()
  }
}

export function matchPath(editor: Editor, path: Path): (node: Node) => boolean {
  const [node] = Editor.node(editor, path)
  return (n) => n === node
}
