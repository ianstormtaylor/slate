...
export const isAtStartOfBlock = (fn, editor) => point => {
...
export const isAtStartOfPath = (fn, editor) => (point, path) => {
  const { value: { document } } = editor
  const [, firstPath] = document.firstText({ path })
  return point.offset === 0 && point.path.equals(firstPath)
}

export const isAtEndOfPath = (fn, editor) => (point, path) => {
  const { value: { document } } = editor
  const [firstNode, firstPath] = document.firstText({ path })
  return point.offset === firstNode.text.length && point.path.equals(firstPath)
}

export const isAtEdgeOfPath = (fn, editor) => (point, path) => {
  return (
...
export const getNextPoint = (fn, editor) => (point, options = {}) => {
...
export const getNextCharacterPoint = (fn, editor) => point => {
  const { value: { document } } = editor
  const { path, offset } = point
...
export const getNextNonVoidPoint = (fn, editor) => point => {
  const { value: { document } } = editor
  let next = point
...
export const getNextWordPoint = (fn, editor) => point => {
  const { value: { document } } = editor
  const { path, offset } = point
  const [block, blockPath] = document.closestBlock(path)
  const relativePath = path.slice(blockPath.size)
...
export const getPreviousPoint = (fn, editor) => (point, options = {}) => {
  const { allowZeroWidth = false } = options
  const { value } = editor
  const { document } = value
  const { path, offset } = point
...
export const getPreviousCharacterPoint = (fn, editor) => point => {
  const { value: { document } } = editor
  const { path, offset } = point
  const [block, blockPath] = document.closestBlock(path)
  const relativePath = path.slice(blockPath.size)
...
export const getPreviousNonVoidPoint = (fn, editor) => point => {
  const { value: { document } } = editor
  let prev = point
...
export const getPreviousWordPoint = (fn, editor) => point => {
  const { value: { document } } = editor
  const { path, offset } = point
  const [block, blockPath] = document.closestBlock(path)
  const relativePath = path.slice(blockPath.size)
