...
class Editor extends React.Component {
  ...
  static propTypes = {
    ...
    onReceiveState: React.PropTypes.func,
  ...
  constructor(props) {
    ...
    this.state.state = this.onReceiveState(props.state)
  ...
  onReceiveState = (state) => {
    ...
    for (const plugin of this.state.plugins) {
      if (!plugin.onReceiveState) continue
      const newState = plugin.onReceiveState(state, this)
      if (newState == null) continue
      state = newState
    }

    if (!this.props.onReceiveState) return state
    return this.props.onReceiveState(state)
  }
